    ...
    <div class="panel panel-default">
        ...
        {{-- <div class="row w3-res-tb">
            ...
            <select class="input-sm form-control w-sm inline v-middle">
                @foreach ($lietke_pdbtheoten as $key => $ten)
                <option value="{{$ten->IdKH}}">{{$ten->TenKH}}</option>
                @endforeach
            </select>
                ...
                <tbody>
                    ...
                    <td>
                        @if ($phieudatban->TinhTrang == 0)
                            <a onclick="return confirm('Bạn có chắc chắn duyệt không?')" href="{{ route('Admin.ban', ['id'=>$phieudatban->IdDatBan]) }}" title="Click để duyệt"><span class="badge badge-warning">Chưa duyệt</span></a>
                            <br>
                            <a onclick="return confirm('Bạn có chắc chắn huỷ phiếu?')" href="{{ route('Admin.ban2', ['id'=>$phieudatban->IdDatBan]) }}" title="Click để huỷ phiếu"><span style="color:red">Huỷ phiếu</span></a>
